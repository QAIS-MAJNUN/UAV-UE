...
from rpg.nms import non_max_suppression
...
delta = 5
# 图片存储主路径
PREPATHNAME = "E:\\UAV-temp-staging\\Capture-Process\\"
...
def imageprocess(pictureid):
    # 加载图片
    pattern = segmentationpath + "Segmentation" + str(pictureid) + f"_*_*_*" + ".png"
    ...
    # print(image_path)
    pattn = r"Segmentation\d+_(\d+)_(\d+)_(\w+)\.png"
    ...
    print("处理后数据")
    new_car_list = non_max_suppression(car_list)

    for p in new_car_list:
        print(p.name, p.xmin, p.ymin, p.xmax, p.ymax)

    print("此次共去除%d张标注" %(len(car_list) - len(new_car_list)))
    new_car_list = car_list
    # 输出到标签中
    # print(pictureid)
    # print(new_car_list)
    pathname = scenepath + imgpth + ".png"
    picture = Picture(pathname, new_car_list, pictureinfo)
