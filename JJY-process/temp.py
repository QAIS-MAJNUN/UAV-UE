...
# 拍摄数据集 capture_num:用来指定拍摄多少的高度
def image_capture(picture_num,capture_num):
# 从capture_num个不同的高度拍摄: 10*n
    for j in range(capture_num):
        ...
        for i in range(12):
            ...
            found6 = AirSim_client.simSetSegmentationObjectID("BoxTruck[\w]*", 48, True)
            found7 = AirSim_client.simSetSegmentationObjectID("Jeep[\w]*", 109, True)
            found8 = AirSim_client.simSetSegmentationObjectID("Motor[\w]*", 178, True)
            found9 = AirSim_client.simSetSegmentationObjectID("RoadRoller[\w]*", 189, True)
            found10 = AirSim_client.simSetSegmentationObjectID("SUV[\w]*", 172, True)



            # 若此次拍照没有拍到车辆，则删除一系列照片
            if(not (found1 or found2  or found4 or found5 or found6 or found7 or found8 or found9 or found10)):
                photo_path = scenepath + "Scene" + str(picture_nums + picture_num) + "_" + str(height) + "_" + str(i * 5) + ".png"
...
while True:
    ...
    # # 根据 'P' 按键来拍摄数据集
    # if scan_wrapper[pygame.K_n] and key_counter == 0:
    #     picture_num = image_capture(picture_num)  # 拍摄图像均为1080P
    #     key_counter = 3

    # 根据 'num' 按键来看此次拍摄枚举的高度
    # eg: 按下 '5' 拍摄 0 10 20 30 40
    for i in range(pygame.K_0, pygame.K_9 + 1):
        if scan_wrapper[i] and key_counter == 0:
            picture_num = image_capture(picture_num,i - pygame.K_0)  # 拍摄图像均为1080P
            key_counter = 3
